...
def cifparse(flist, cate):
    ...
    if (loop == 0):
        for i in range(start, nline):
            ...
            elif (line[i][0] == '#' or line[i][:5] == 'loop_'
                  or (line[i][0] == '_' and cate not in line[i][:clen])):
                break
        ...
        while j < nline:  # through list from start position
            if (line[j][0] == ';'):
                ...
                continue
            elif (line[j][0] == '#' or line[j][:clen] == "loop_"
                  or (line[j][0] == '_' and '.' in line[j])):
                break
            else:
    ...
    for i in range(nline):
        ...
        if (uatom and ucomp and uasym and useq):
            ...
            if (n < len(ucomp) and ucomp[n] == comp[i] and uatom[n] == atom[i] and uasym[n] == asym[i] and useq[n] == seq[i]):
                ...
                ss = 'ANISOU%5d %4s%1s%3s%2s%4d%1s %7.0f%7.0f%7.0f%7.0f%7.0f%7.0f%2s%-4s%2s  \n' % (nat, atomname, alter, comp[i], asym2, nres, inst,
                                                                                                    u[0], u[1], u[2], u[3], u[4], u[5], space,
                                                                                                    asym[i], symbol[i])
                fw.write(ss)
                n = n + 1
...
def add_tls(fw, flist):
    ...
    if not (is_a_number(tlsid[-1]) and is_a_number(tlsid1[-1])
            and tlsid[-1] == tlsid1[-1]):
...
def add_remark285(fw, flist):
    ...
    if items:
        ...
        if not (b11 and b12 and b13 and t1 and b21 and b22 and b23 and t2
                and b31 and b32 and b33 and t3):
            return
        n = 0
...
def add_ncs(fw, flist):
    ...
    if items:
        ...
        if not (id and code and b11 and b12 and b13 and t1 and b21 and b22
                and b23 and t2 and b31 and b32 and b33 and t3):
            return
        for i in range(len(b11)):
            idd = ' '
            if code[i] == 'given':
