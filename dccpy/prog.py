...
def getpdb_na(fp, id):
    ...
    for x in fp:
        ...
        if id == 'sugar' and comp in util.dna_rna():
            if not (" C1'" in atom or " C2'" in atom or " C3'" in atom
                    or " O4'" in atom or " C4'" in atom or " C5'" in atom
                    or " O2'" in atom):  # sugar
            ...
            if not (' P  ' in atom or ' OP1' in atom or ' OP2' in atom or " O3'" in atom
                    or ' OP3' in atom or " O5'" in atom):  # phosphate
                t = ''.join([x[:54], '  0.00', x[60:]])
                fw.write(t)
            else:
                fw.write(x)
            ...
            if (' P  ' in atom or ' OP1' in atom or ' OP2' in atom or " O3'" in atom
                or ' OP3' in atom or " O5'" in atom
                or " C1'" in atom or " C2'" in atom or " C3'" in atom
                or " O4'" in atom or " C4'" in atom or " C5'" in atom
                    or " O2'" in atom):  # phosphate
                t = ''.join([x[:54] , '  0.00', x[60:]])
                fw.write(t)
            else:
                fw.write(x)
    ...
    for ch in record.keys():
        ...
        for i, n in enumerate(nres) :
            ...
            for x in atoms:
                ...
                atom = x[12:16]
                if etype == 'prot':
                    if (' N  ' in atom or ' CA ' in atom or ' C  ' in atom
                            or ' O  ' in atom or ' CB ' in atom):
                        fw1.write(x)  # main chain
                    else:
                        fw2.write(x)  # sidechain

                elif etype == 'na':
                    if (' P  ' in atom or ' OP1' in atom or ' OP2' in atom
                            or ' OP3' in atom or " O5'" in atom):  # phosphate
                        phos.append(x)
                    ...
                    elif (" C1'" in atom or " C2'" in atom or " C3'" in atom
                          or " O4'" in atom or " C4'" in atom or " C5'" in atom
                          or " O2'" in atom):  # sugar
                        fw1.write(x)
                    else:
...
def run_refmac_more(pdb, mtz, dic, dici):
    ...
    if (not (util.is_number(dic['rfact']) and util.is_number(dici['rfact']))
            or float(dici['rfact']) - float(dic['rfact']) < rcut):
        return '', '', '', '', dicout
    ...
    for i, x in enumerate(option):
        ...
        if (util.is_number(dicx['fom']) and float(dicx['fom']) > 0.6
            and util.is_number(dicx['rfact']) and util.is_number(dic['rfact'])
                and float(dicx['rfact']) - float(dic['rfact']) < rcut):

            if '-twin ' in option[i] and float(dici['rfact']) - float(dic['rfact']) > 0.05 \
               and '2:' in dic['twin_fr']:
