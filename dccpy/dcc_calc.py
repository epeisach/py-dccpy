...
Simple Usage:   dcc  -pdb  xyzfile  -sf  sffile
  ...
  Running multiple program together(sfcheck, refmac, cns, phenix_x):
    ...
    if dic["ligmap"] and not dic["ligmapcif"]:
        if (not dic["sdsc_map"] and not check_ligand(pdbfile, 0)) or (dic["sdsc_map"] and not check_ligand(pdbfile, 1)):
...
def get_argument(arg, dic):
    ...
    if not (dic["sfcheck"] or dic["phenix_x"] or dic["phenix_n"] or dic["refmac"] or dic["phenix_xn"] or dic["cns"] or dic["shelx"] or dic["auto"] or dic["fem"]):
...
def check_env(dic):
    ...
    if dic["phenix_xtriage"] or dic["phenix_x"] or dic["phenix_n"] or dic["exp"] == "n" or dic["exp"] == "xn" or dic["phenix_xn"] or dic["fem"]:
...
def check_xyz(dic):
    ...
    for x in fp:
        ...
        if (("UNL" in x[17:20] or "UNK" in x[17:20] or "UNX" in x[17:20]) and (" U" in x[76:78] or "  " in x[76:78])) or " X" in x[76:78]:
...
def do_validation_auto(dic, pdb_new, sf_new):
    ...
    cond_cut = util.is_number(dic["rfact"]) and util.is_number(dd["rfact"]) and (float(dd["rfact"]) - float(dic["rfact"]) <= rcut)
    if cond_cut:
        return dd_all
    ...
    for x in prog_test:
        ...
        cond_cut = util.is_number(dic["rfact"]) and util.is_number(dd_in["rfact"]) and (float(dd_in["rfact"]) - float(dic["rfact"]) <= rcut)
...
def do_validation(dic, pdb_new, sf_new):
    ...
    if dic["refmac"]:
        ...
        print("Rwork/Rfree: reported=%s/%s, calculated=%s/%s, fom=%s " % (dic["rfact"], dic["rfree"], dic3["rfact"], dic3["rfree"], dic3["fom"]))
...
def write_all(dic, all_dic):
    ...
    if not dic["phenix_xn"] and dic["exp"] != "xn" and dic["exp"] != "nx":
        ...
        fw.write(
            "1 %8s %6s %6s %6s %6s %6s %6s %6s %6s ?  ?  ?  %8s 'Reported Xray'\n"
            % (dic["prog"], dic["resh"], dic["resl"], dic["rall"], dic["rfact"], dic["rfree"], dic["nref"], dic["comp"], dic["nfree"], version)
        )

        fw.write(
            "2 %8s %6s %6s %6s %6s %6s %6s %6s %6s ?  ?  ?  %8s 'Reported Neut'\n"
            % (dic["prog"], dic["resh_n"], dic["resl_n"], dic["rall_n"], dic["rfact_n"], dic["rfree_n"], dic["nref_n"], dic["comp_n"], dic["nfree_n"], version)
        )
        np = 2
...
def best_of_solutions(dd):
    ...
    if len(dd) == 2:
        if util.is_number(dd[0]["rfact"]) and util.is_number(dd[1]["rfact"]) and float(dd[0]["rfact"]) > float(dd[1]["rfact"]) + 0.001:
            n = 1
...
def add_lldf_to_outfile(dic, ofile):
    ...
    for i, x in enumerate(flist):  # writing
        fw.write(x)
        if "#" in x[:10] and "Overall properties from" in flist[i + 1] and len(all_lldf) > 0:
            cifhead2 = write_cif.dcc_ciftoken("pdbx_non_poly", dic)
            fw.write(cifhead2)

            for y in all_lldf:
                t = "%4s %s %s %4s %s %s %s %s %s %s %s\n" % (y[8], y[1], y[2], y[0], y[3], y[4], y[5], y[9], y[11], y[6], y[7])
                fw.write(t)
            fw.write("#\n")
...
def add_modelid_to_outfile(dic, ofile):  # pylint: disable=unused-argument
    ...
    for i, x in enumerate(fp):
        if i > 0 and (("rscc_mapman." in fp[i - 1] and "_mapman." not in x) or ("_sfcheck." in fp[i - 1] and "_sfcheck." not in x)):
            firstl = x
            break
    if not firstl:
        return
...
def add_pdbid_to_outfile(dic, file):
    ...
    for ln in lis:
        ...
        if n == 0 and "_pdbx_rscc_mapman." in tmp[0]:
            ...
            if tmp[4] == end[2].strip() and tmp[2] == end[1].strip() and tmp[3] == end[0].strip():
                j = j + 1
                if j >= len(pdbid):
                    j = len(pdbid) - 1
                end = resid[pdbid[j]][1]
...
def check_sf(dic):
    ...
    n1 = 0
    for line in fp:
        if " x " in line or " h " in line or " l " in line or " < " in line or " - " in line:
            continue
    ...
    if dic["exp"] == "xn":  # X-ray & Neutron diffr.
        ...
        if n2 < 100:
            util.perror(
                "Error: Only one data set in SF file.\n \
...
def check_pdb(pdbfile, dic):
    ...
    for line in fr:
        ...
        if line[0:6] == "HEADER":
            ...
            if ("X-RAY" in line[6:] or "XRAY" in line[6:] or "X_RAY" in line[6:]) and "NEUTRON" not in line[6:].upper():
                dic["exp"] = "x"
        ...
        elif "REMARK   3   NUMBER OF REFLECTIONS             " in line or "NUMBER OF REFLECTIONS   (NO CUTOFF)" in line:
            if not neut:
        ...
        elif ("REMARK   3   R VALUE" in line[:21] and "(WORKING + TEST SET)" in line[22:47]) or ("R VALUE   (WORKING + TEST SET, NO CUTOFF) :" in line):
            if not neut:
                dic["rall"] = util.get_value_after_id(line, ":")
            if neut:
                dic["rall_n"] = util.get_value_after_id(line, ":")

        elif ("REMARK   3   R VALUE" in line[:21] and " (WORKING SET)" in line[:47]) or ("R VALUE          (WORKING SET, NO CUTOFF) : " in line):
            if not neut:
                dic["rfact"] = util.get_value_after_id(line, ":")
            if neut:
                dic["rfact_n"] = util.get_value_after_id(line, ":")

        elif "REMARK   3   FREE R VALUE                    " in line or "FREE R VALUE                  (NO CUTOFF" in line:
            if not neut:
        ...
        elif "REMARK   3   FREE R VALUE TEST SET COUNT    " in line or "FREE R VALUE TEST SET COUNT   (NO CUTOFF)" in line:
            if not neut:
        ...
        elif "REMARK   3" in line and ("  FRACTION:" in line or "TWIN FRACTION :" in line):
            fract = line.split(":")[1]
        ...
        elif ("ATOM" in line[:4] or "HETA" in line[:4] or "ANISOU" in line[:6]) and len(line.strip()) > 64:
            if "**" in line:
            ...
            if (("UNL" in line[17:20] or "UNK" in line[17:20] or "UNX" in line[17:20]) and (" U" in line[76:78] or "  " in line[76:78])) or " X" in line[76:78]:
            ...
            elif " D" in line[76:78] and "exp" in dic and ((dic["exp"] == "x" or dic["exp"] == "e") and dic["phenix_n"] == 0 and dic["phenix_xn"] == 0):
    ...
    if nmat > 0 and len(bmat) == 0:
        ...
        for i, mt in enumerate(mtrix):
            ...
            if mt[0][0] == 1.0 and mt[1][1] == 1.0 and mt[2][2] == 1.0 and mt[0][3] == 0.0:
                continue
...
def coord_by_biomt(pdbfile, m285, btrix, bmat):
    ...
    for x in fp:
        if x[:4] == "ATOM" or x[:6] == "HETATM":
            atom.append(x)
        elif "CONECT" not in x[:6] and "MASTER" not in x[:6] and "END" not in x[:3]:
            fw.write(x)
            pdbhead.append(x)
...
def initialize_dic():
    ...
    for x in dd:
        dic[x] = "?"
    dic["pdbid"] = "xxxx"
    dd = ["sf_nobs", "sf_nwork", "sf_nfree", "sf_pfree", "sf_nfpair", "sf_nfpair_p", "sf_nfpair_m", "sf_nfpair_pm", "sf_nipair", "sf_nipair_p", "sf_nipair_m", "sf_nipair_pm"]
    for x in dd:
        dic[x] = ""
    return dic
...
def check_result(d1, d2):
    ...
    if util.is_number(d1["resh"]) and float(d1["resh"]) > 1.7 and d1["tls_report"] == "N" and d1["anis"] == "Y" and d1["model"] == "?":
        info.append("Warning: ANISOU records exists but no TLS records (resolution=%sA)." % d1["resh"])

    if (
        util.is_number(d1["resh"])
        and float(d1["resh"]) < 1.5
        and d1["tls_report"] == "Y"
        and util.is_number(d1["rfact"])
        and util.is_number(d2["rfact"])
        and float(d2["rfact"]) - float(d1["rfact"]) > 0.015
        and "REFMAC" in d1["prog"].upper()
    ):
    ...
    if util.is_number(d1["nfree"]) and util.is_number(d2["nfree"]) and util.is_number(d1["resh"]) and util.is_number(d2["resh"]):  # Nfree
    ...
    for i in range(0, m):
        if table in cif[i].strip()[: len(table) + 1]:
            ...
            if "loop_" in cif[i - 1]:
                n1 = i - 1
            for j in range(i, m):
                if "#" in cif[j][:1] or "loop_" in cif[j][:5] or ("_" in cif[j][:1] and table not in cif[j]):
                    n2 = j
                    break
            if n2 > n1:
                break
    ...
    if id == 0 or id == 1:
        for k, v in ch_lig.items():
            for y in v:
                natm = 0
                for x in pdb:
                    if ("ATOM" in x[:4] or "HETATM" in x[:6]) and k == x[20:22].strip() and int(x[22:26]) == y:
                        natm = natm + 1
                        # print natm, k, v, x
                        if natm > 1:
                            return 1
