...
import math
# from config import *
from util import perror, is_digit, delete_file, get_value_after_id, is_number, myreplace_case
...
def check_tls(pdb, id):
    ...
    for i, x in enumerate(tls):
        ...
        elif (('REMARK   3    RESIDUE RANGE :' in x[:29])
              or ('REMARK   3    SELECTION:' in x[:29]
                  and 'buster' in prog.lower() and '{' not in x[20:])):
...
def check_residue_range(all_range):
    ...
    for i in range(0, nr):
        ...
        for j in range(i + 1, nr):
            ...
            ch = all_range[j][0]
            if ((n1 <= k1 and n2 >= k2)     # 1
                or (k1 <= n1 and k2 >= n2)     # 2
                or (k1 >= n1 and n2 >= k1 and k2 >= n2)  # 3
                    or (n1 >= k1 and k2 >= n1 and n2 >= k2)):  # 4
    ...
    for i, x in enumerate(tls):
        if 'all' in x:
            ...
            # elif (pattern(i, tls, 'chain', '?') and (nn==2 or nn==4)) : #one chain
                  ...
                  or pattern(i, tls, 'chain', '?', ')')):  # one chain
...
def tlsxc_origin(pdbfile, info):
    ...
    for i, x in enumerate(fp):
        ...
        elif (('REMARK   3    SET :' in x[:20])
              or ('REMARK   3    SELECTION:' in x[:29] and 'buster' in prog.lower())):  # buster
            nbu = nbu + 1
            if nbu == 1 and info == 1:
...
def precheck_tls(pdbfile):
    ...
    for x in fr:
        ...
        if "REMARK   3    RESIDUE RANGE : " in x:
            tmp = x.split(':')[1].split()
            if len(tmp) != 4 or is_digit(tmp[1]) == 0 or is_digit(tmp[3]) == 0 \
               or (int(tmp[1]) < 0 and (int(tmp[3]) == -1 or int(tmp[3]) >= 999)):
                wk = 0
                break
...
def get_tlsinp(pdbfile):
    ...
    n = 0
    for ln in fr:
        if 'BULK SOLVENT MODELLING' in ln or ln[0:4] == 'ATOM' or ln[0:6] == 'HETATM':
            break
...
def convert_range_phenix_to_refmac(ln, chain_range):
    ...
    if n == 2:
        ...
        if 'CHAIN' in tmp[0] and len(tmp[1]) == 1 \
           and 'AND' in tmp[2] and 'RES' in tmp[3]:

            res = resi_range(tmp[4])
            v.append(res)
            val[tmp[1]] = v

        elif ('RES' in tmp[0] and len(tmp[4]) == 1
              and 'AND' in tmp[2] and 'CHAIN' in tmp[3]):
...
def correct_tls(pdbfile):
    ...
    # print(res_range[4])
    range1 = {}
    _prog, remark_b, tls_b, anisou, ntls = '?' * 5
    for line in fr:
...
def correct_residue_range(ntls, pdbfile, line, res_range):
    ...
    elif (nr == 2 and is_digit(ntls) > 0 and int(ntls) == 1
          and is_digit(res[0]) > 0 or is_digit(res[1]) > 0):
...
def correct_range(pdbfile, res1, res2, ln, ch, res_range):
    ...
    elif (ch not in chain_range.keys() and ch not in chain_range1.keys()
          and ch not in chain_range2.keys()):
    ...
    if (ch in chain_range.keys()
        and (n1 in chain[ch] or n2 in chain[ch]
             or (n1 < chain_range[ch][0] and n2 > chain_range[ch][1]) and n2 <= 999)):  # polymer
